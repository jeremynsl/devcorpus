...
def setup_test_env():
    ...
    test_db = os.path.join(
        tempfile.gettempdir(), f"chroma_test_gradio_{int(time.time())}"
    )
    ChromaHandler.configure(test_db)
    ChromaHandler.reset()  # Reset the singleton before each test

    yield

    # Clean up after test
    ChromaHandler.reset()  # Reset the singleton after each test
...
def gradio_chat(mock_config, mock_chat_interface, mock_chroma):
    ...
    with (
        patch("scraper_chat.ui.gradio_app.config", mock_config),
        patch(
...
async def test_chat_streaming(gradio_chat):
    ...
    async for new_history, refs in gradio_chat.chat(
        "test message", history, ["collection1"], "test-model"
    ):
    ...
    assert responses[-1] == "Test response", (
        f"Expected 'Test response', got '{responses[-1]}'"
    )
...
def test_refresh_databases(gradio_chat):
    ...
    with patch(
        "scraper_chat.database.chroma_handler.ChromaHandler._instance", mock_handler
    ):
...
def test_delete_collection_error(gradio_chat):
    ...
    mock_handler = MagicMock()
    mock_handler.delete_collection.return_value = False
    mock_handler.get_available_collections.return_value = ["collection1", "collection2"]
    ...
    with patch(
        ...
        mock_handler.delete_collection.assert_called_once_with("collection1")
