...
async def fetch_github_content(url: str) -> dict:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.get(f"https://api.github.com/repos/{owner}/{repo}", headers=headers) as response:
            ...
            if response.status == 200:
                ...
                return {
                    ...
                    "files": [
                        ...
                        if f["type"] == "blob" and f["path"].endswith(
                            (".md", ".js", ".ts", ".py", ".jsx", ".tsx", ".vue", ".svelte", ".html", ".css")
                        )
                    ]
                }
            else:
...
async def scrape_recursive(start_url: str, user_agent: str, rate_limit: int, use_db: bool = False):
    ...
    if not start_url.startswith(('http://', 'https://')):
        ...
        if "github.com" in start_url and "/blob/" not in start_url:
            ...
            progress_bar = tqdm(desc="Files Processed", unit="files", total=len(repo_info["files"]))
